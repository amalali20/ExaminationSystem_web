...
function isValidatName()
{
    return namePattern.test(nameFiled.value)
}
...
function checkFormValidity() {
    if (isValidatName() && isValidatEmail() && isValidatPass()) {
...
nameFiled.addEventListener('blur', function(){
    if(!isValidatName())
    {
        ...
        warningName.innerText = 'Enter valid name !!!';
...
passField.addEventListener('blur', function(){
    ...
    {
        this.style.border = '2px solid red';
        warningPass.innerText = 'must bt more than 8 char !!!';
...
function updateProgressBar(percentage) {
    ...
    thanks.innerText = `Thanks ${nameFiled.value}, for your response`;
